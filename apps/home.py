...
#############################
    ...
    #     create_image_table()
    #     #   add_data_to_image_to_table(id_post,user_id,img,width,height)
    #     add_data_to_image_to_table(1,1,data,width,height)
    #     #st.write("test type ",type))
    ...
    #     oimg = convert_bytes_to_img(width,height,arr)
    #     st.image(oimg)
    # #   blog_table_id TEXT,id_post TEXT,user_id TEXT,img BLOB,width INTEGER,height INTEGER
    # df = pd.read_sql_query('SELECT * FROM table_Image WHERE user_id = "{}"'.format(1),conn)
    # df_new = df[['blog_table_id','id_post','user_id','img','width','height']]
    # data = df_new['img'][0]
    # h = int(df_new['height'][0])
    # w = int(df_new['width'][0])

    # st.write("type asfter",type(data))
    # nova_lista = marshal.loads(data)  
    # arr = np.array(nova_lista)
    # st.write("PRINTTTTTTTTT type asfter",type(arr)) 
    # st.write(arr)
